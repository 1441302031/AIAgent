    ...
    def stream_complete(self, request: CompletionRequest) -> Iterator[CompletionEvent]:
        ...
        try:
            with self.client.stream("POST", "/chat/completions", headers=headers, json=payload) as response:
                ...
                if 400 <= response.status_code:
                    raise ProviderError(
                        _error_message(response, f"Moonshot request failed with status {response.status_code}.")
                    )
                ...
                try:
                    ...
                    for line in response.iter_lines():
                        ...
                        if not line.startswith("data:"):
                            raise ProviderError("Moonshot returned an invalid streaming response.")
                    ...
                    raise TransportError("Moonshot stream failed.") from exc

                if not saw_done:
                    raise ProviderError("Moonshot streaming response ended before [DONE].")
...
def _parse_stream_event(data: str) -> CompletionEvent | None:
    try:
        ...
        raise ProviderError("Moonshot returned an invalid streaming response.") from exc
